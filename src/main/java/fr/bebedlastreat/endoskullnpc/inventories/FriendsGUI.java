...
public class FriendsGUI extends CustomGui {
    ...
    public FriendsGUI(Player p, int page) {
        ...
        int i = 0;
        setItem(slots[i], new CustomItemStack(Material.BARRIER).setName("§cVous n'avez pas d'amis"));
        for (UUID uuid : friends) {
            String name = SpigotPlayerInfos.getNameFromUuid(uuid);
            boolean online = FriendUtils.isOnline(uuid);
            setItem(slots[i], CustomItemStack.getPlayerSkull(name).setName(EndoSkullAPI.getPrefix(uuid) + name + " " + (online ? "§a(Connecté)" : "§c(Déconnecté)")));
            i++;
